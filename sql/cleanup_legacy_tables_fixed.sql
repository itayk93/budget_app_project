-- Cleanup Legacy and Unnecessary Tables - FIXED VERSION
...
BEGIN
    ...
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'monthly_budget') THEN
        ...
        SELECT count(*) INTO row_count_var FROM monthly_budget;
        RAISE NOTICE 'monthly_budget contains % rows', row_count_var;
    ELSE
    ...
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'monthly_savings') THEN
        ...
        -- Show row count for reference
        SELECT count(*) INTO row_count_var FROM monthly_savings;
        RAISE NOTICE 'monthly_savings contains % rows', row_count_var;
    ELSE
    ...
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'category_order_backup') THEN
        ...
        -- Show row count for reference
        SELECT count(*) INTO row_count_var FROM category_order_backup;
        RAISE NOTICE 'category_order_backup contains % rows', row_count_var;
    ELSE
    ...
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'stock_prices_backup') THEN
        ...
        -- Show row count for reference
        SELECT count(*) INTO row_count_var FROM stock_prices_backup;
        RAISE NOTICE 'stock_prices_backup contains % rows', row_count_var;
    ELSE
